...
// static void xr20m117x_ier_set(struct uart_port *port, u8 bit)
// {
// 	struct xr20m117x_port *s = dev_get_drvdata(port->dev);
// 	struct xr20m117x_one *one = to_xr20m117x_one(port, port);
...
static void xr20m117x_handle_tx(struct uart_port *port)
{
	...
	xr20m117x_fifo_write(port, txlen);
	...
	if (kfifo_is_empty(&tport->xmit_fifo))
		xr20m117x_stop_tx(port);
	// else
	// 	xr20m117x_ier_set(port, XRM117X_IER_THRI_BIT);
	uart_port_unlock_irqrestore(port, flags);
}
...
static void xr20m117x_reconf_rs485(struct uart_port *port)
{
	...
	if (rs485->flags & SER_RS485_ENABLED) {
		...
		if (rs485->flags & SER_RS485_RTS_ON_SEND)	// Invert if send is high
...
static int xr20m117x_config_rs485(struct uart_port *port, struct ktermios *termios, struct serial_rs485 *rs485)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 2, 0)
	const u32 mask = XRM117X_EFCR_AUTO_RS485_BIT | 
									XRM117X_EFCR_RTS_INVERT_BIT;
			   ...
			   unsigned long arg)
{
		...
		case TIOCSRS485:
			if (copy_from_user(&rs485, (void __user *)arg, sizeof(rs485)))
				return -EFAULT;	
			struct ktermios *termios = &port->state->port.tty->termios;
			xr20m117x_config_rs485(port, termios,&rs485);
			return 0;
		case TIOCGRS485:
...
out_uart:
	...
	if (dev->of_node) {
		...
		of_property_for_each_u32(dev->of_node, "irda-mode-ports", u)
...
static void xr20m117x_i2c_remove(struct i2c_client *client)
{
	xr20m117x_remove(&client->dev);
}
